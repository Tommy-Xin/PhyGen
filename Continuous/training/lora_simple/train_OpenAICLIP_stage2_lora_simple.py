...
from lora_simple.vit_dynamic_lora import apply_dynamic_vit_lora
from copy import deepcopy

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))
from evaluation.evaluate_mmvp_OpenAICLIP_224_eval import evaluate_checkpoint
from add_processor_tokenizer import add_processor_tokenizer
...
def _build_tracker_hparams(args, total_batch_size):
    return {
        "model_name": str(args.model_name),
        "clip_image_size": int(args.clip_config.clip_image_size),
        "learning_rate": float(args.learning_rate),
        "lr_scheduler": str(args.lr_scheduler),
        "max_train_steps": int(args.max_train_steps),
        "gradient_accumulation_steps": int(args.gradient_accumulation_steps),
        "per_device_train_batch_size": int(args.train_batch_size),
        "global_train_batch_size": int(total_batch_size),
        "mixed_precision": str(args.mixed_precision),
        "load_step": int(args.load_step),
        "lora_r_min": int(args.lora_config.r),
        "lora_r_max": int(args.lora_simple.max_rank),
        "lora_alpha": int(args.lora_config.lora_alpha),
        "lora_dropout": float(args.lora_config.lora_dropout),
        "lora_last_n_layers": int(args.lora_simple.last_n_layers),
    }


def _save_hparams_to_output_dir(args):
    raw_cfg = OmegaConf.to_container(args, resolve=False)
    resolved_cfg = OmegaConf.to_container(args, resolve=True)

    raw_yaml_path = os.path.join(args.output_dir, "hparams_raw.yaml")
    resolved_yaml_path = os.path.join(args.output_dir, "hparams_resolved.yaml")
    resolved_json_path = os.path.join(args.output_dir, "hparams_resolved.json")

    OmegaConf.save(config=OmegaConf.create(raw_cfg), f=raw_yaml_path)
    OmegaConf.save(config=OmegaConf.create(resolved_cfg), f=resolved_yaml_path)
    with open(resolved_json_path, "w", encoding="utf-8") as f:
        json.dump(resolved_cfg, f, ensure_ascii=False, indent=2)

    logger.info("Saved hyperparameters to %s, %s and %s", raw_yaml_path, resolved_yaml_path, resolved_json_path)
...
def main():
    ...
    # set LoRA: only inject into the last ViT layers on q/k/v/o + mlp fc1/fc2 with dynamic rank
    clip_vis.model, layer_range, rank_pattern = apply_dynamic_vit_lora(
        model=clip_vis.model,
        lora_r=args.lora_config.r,
        lora_alpha=args.lora_config.lora_alpha,
        lora_dropout=args.lora_config.lora_dropout,
        lora_bias=args.lora_config.bias,
        last_n_layers=args.lora_simple.last_n_layers,
        max_rank=args.lora_simple.max_rank,
    )
    logger.info(
        "Applied dynamic ViT LoRA on layers [%s, %s], rank range [%s, %s], target modules=%s",
        layer_range[0],
        layer_range[1],
        min(rank_pattern.values()),
    ...
    tracker_hparams = _build_tracker_hparams(args, total_batch_size)

    if accelerator.is_main_process:
        accelerator.init_trackers(args.tracker_project_name, tracker_hparams)
        _save_hparams_to_output_dir(args)
        logger.info("Tracker hyperparameters: %s", tracker_hparams)
    ...
    eval_benchmark_dir = getattr(args, "eval_benchmark_dir", None)
    eval_delete_ckpt = bool(getattr(args, "eval_delete_ckpt", True))
    eval_enabled = eval_steps > 0 and bool(eval_benchmark_dir)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if eval_enabled and global_step % eval_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        logger.info("Saved eval ckpt to %s", save_path)

                        try:
                            add_processor_tokenizer(save_path)
                            csv_output_path = os.path.join(args.output_dir, f"mmvp_predictions_step_{global_step}.csv")
                            ...
                            results_path = os.path.join(args.output_dir, "mmvp_eval_results.jsonl")
                            with open(results_path, "a", encoding="utf-8") as f:
                                f.write(json.dumps({"step": global_step, "results": results}, ensure_ascii=False) + "\n")
                            logger.info("Saved MMVP result at step %s to %s", global_step, results_path)
                        finally:
                            if eval_delete_ckpt and os.path.isdir(save_path):
                                shutil.rmtree(save_path)
                                logger.info("Removed eval ckpt %s", save_path)
