...
def _compute_clip_contrastive_loss(image_features, text_features, temperature):
...
def main():
    ...
    # set LoRA: align with lora_simple dynamic injection strategy
    ...
    clip_base_path = os.path.join(repo_root, "pretrained_weights", "clip-vit-large-patch14")
    clip_tokenizer = CLIPTokenizer.from_pretrained(clip_base_path)
    clip_loss_weight = float(getattr(args, "clip_loss_weight", 0.005))
    clip_loss_temperature = float(getattr(args, "clip_loss_temperature", 0.07))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(clip_vis):
                ...
                loss_diff = F.mse_loss(model_pred.float(), (x_0 - x_1).float(), reduction="mean")
                with torch.no_grad():
                    ...
                    text_features = clip_vis.model.get_text_features(**text_inputs)
                loss_clip = _compute_clip_contrastive_loss(
                    inp["vec"],
                    text_features,
                    clip_loss_temperature,
                )
                loss = loss_diff + clip_loss_weight * loss_clip
            ...
            if accelerator.sync_gradients:
                ...
                # stored ckpts in the middle of training
                if global_step == args.max_train_steps // 2:
                    if accelerator.is_main_process:
                        ...
                        processor = CLIPImageProcessor.from_pretrained(base_model_path)
                        tokenizer = CLIPTokenizer.from_pretrained(base_model_path)
            ...
            if global_step >= args.max_train_steps:
                if accelerator.is_main_process:
                    if args.clip_config.clip_image_size == 336:
                        ...
                        save_path = os.path.join(args.output_dir, f"clip-vit-large-patch14-{global_step}")
                        base_model_path = '/home/gaiyiming/hjq/xinc/GenHancer/Continuous/pretrained_weights/openai-clip-large-patch14'
                    unwrapped_clip_vis = accelerator.unwrap_model(clip_vis)
                    save_model = deepcopy(unwrapped_clip_vis.model).merge_and_unload()
                    save_model.save_pretrained(save_path, safe_serialization=False)
