...
def main():
    ...
    eval_delete_ckpt = getattr(args, "eval_delete_ckpt", True)
    eval_enabled = bool(eval_steps and eval_benchmark_dir)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(super_model):
                ...
                with torch.no_grad():
                    ...
                    text_features = super_model.clip_vis.model.get_text_features(**text_inputs)
                    ...
                    if accelerator.is_main_process:
                        ...
                        try:
                            ...
                            csv_output_path = os.path.join(
                                args.output_dir, f"mmvp_predictions_step_{global_step}.csv"
                            )
                            ...
                            with open(results_path, "a", encoding="utf-8") as f:
                                f.write(json.dumps({"step": global_step, "results": results}) + "\n")
                        finally:
                            if eval_delete_ckpt and os.path.isdir(save_path):
                                shutil.rmtree(save_path)
                                logger.info(f"Removed eval ckpt {save_path}")
