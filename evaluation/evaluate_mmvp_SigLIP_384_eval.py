...
def benchmark_model(processor, tokenizer, model, benchmark_dir, device="cpu", csv_output_path=None):
    ...
    if csv_output_path is None:
        csv_output_path = os.path.join(os.getcwd(), "Prediction_Results_SigLIP_384.csv")
...
def evaluate_checkpoint(model_path, benchmark_dir, device=None, csv_output_path=None):
    ...
    vision_tower = SiglipModel.from_pretrained(model_path, device_map=device)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate MMVP for a SigLIP checkpoint")
